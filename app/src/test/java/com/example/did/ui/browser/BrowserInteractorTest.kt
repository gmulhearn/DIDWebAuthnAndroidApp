...
class BrowserInteractorTest : BaseTests() {

    private val coroutineScopeSpy: MSCoroutineScope by before {
        spy(MSCoroutineScope(dispatchers = TestDispatcherProvider()))
    }

    private val interactor: BrowserInteractor by before {
        BrowserInteractor(coroutineScopeSpy)
    }

    private val output by before {
        mock<BrowserContract.InteractorOutput>()
    }
    ...
    fun `interactor test required`() {
        Assert.fail("Interactor unit tests have not been implemented for Browser. Replace this method with actual tests.")
    }
}